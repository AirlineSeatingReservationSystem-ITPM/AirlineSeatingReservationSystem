...
const PaymentLogin = ()=>{
    ...
    const loginHandler = async (e)=>{
        ...
        try {
            ...
            const {data} = await axios.post("http://localhost:8085/api/auth/staff-login-paymentM" , {email , password} , config);

            console.log(data)

            localStorage.setItem("authTokenStaff" , data.token);

            history("/staff-paymentM-home");
