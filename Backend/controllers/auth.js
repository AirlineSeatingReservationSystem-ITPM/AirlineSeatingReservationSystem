...
const paymentStaff = require("../models/paymentStaff");
...
exports.forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ success: true, error: "Email couldn't be sent" });
    }
...
const sendStaffToken = (user, statusCode, res) => {
  //JWT get
...
exports.registerStaffPaymentM = async (req, res, next) => {
  const { email, password } = req.body; //destructure method

  try {
    const staff = await paymentStaff.create({
      email,
      password, //this.password filed of user.js in models
    });
    sendStaffToken(staff, 200, res);
...
exports.loginStaffPaymentM = async (req, res, next) => {
  const { email, password } = req.body;
  ...
  try {
    const staff = await paymentStaff.findOne({ email }).select("+password");
